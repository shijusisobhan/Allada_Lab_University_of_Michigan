...
% Last Modified by GUIDE v2.5 05-Jul-2024 17:56:45
...
set(handles.ref_genotype, 'Enable', 'on');
...
ref_genotype=(get(handles.ref_genotype, 'string'));
...
% Remove the reference genotype from the list
genotypes(strcmp(genotypes, ref_genotype)) = [];
...
ref_data = Result_all_SC(strcmp(Result_all_SC.Genotype, ref_genotype), :);
...
Pval_sleep = sortrows(results, 'Sleep_Pval');
Pval_BN = sortrows(results, 'BoutNumber_Pval');
Pval_BL = sortrows(results, 'BoutLength_Pval');
Pval_TA = sortrows(results, 'TotalActivity_Pval');
Pval_AWM = sortrows(results, 'Activity_wakingMin_Pval');
Pval_Latency = sortrows(results, 'Latency_Pval');
Pval_sleep_lost = sortrows(results, 'SleepLost_Pval');
Pval_per_SL = sortrows(results, 'percetage_SL_Pval');
Pval_SG = sortrows(results, 'SleepGain_Pval');
Pval_per_SG = sortrows(results, 'percentage_SG_Pval');
Pval_Latency_SD = sortrows(results, 'Latency_after_SD_Pval');



Pval_data_headers={"GT_Sleep" "Sleep_Pval" "GT_BN" "BoutNumber_Pval" "GT_BL" "BoutLength_Pval" "GT_TA" "TotalActivity_Pval" "GT_AWM" "Activity_wakingMin_Pval" "GT_LA" "Latency_Pval"...
        "GT_SL" "SleepLost_Pval" "GT_SLP" "percetage_SleepLoss_Pval" "GT_SG" "SleepGain_Pval" "GT_SGP" "percentage_SleepGain_Pval", "GT_Latency_SD" "Latency_SD_Pval"};

    Pval_data_all=[Pval_data_headers;
        Pval_sleep.Genotype num2cell(Pval_sleep.Sleep_Pval)...
        Pval_BN.Genotype num2cell(Pval_BN.BoutNumber_Pval)...
        Pval_BL.Genotype num2cell(Pval_BL.BoutLength_Pval)...
        Pval_TA.Genotype num2cell(Pval_TA.TotalActivity_Pval)...
        Pval_AWM.Genotype num2cell(Pval_AWM.Activity_wakingMin_Pval)...
        Pval_Latency.Genotype num2cell(Pval_Latency.Latency_Pval)...
        Pval_sleep_lost.Genotype num2cell(Pval_sleep_lost.SleepLost_Pval)...
        Pval_per_SL.Genotype num2cell(Pval_per_SL.percetage_SL_Pval)...
        Pval_SG.Genotype num2cell(Pval_SG.SleepGain_Pval)...
        Pval_per_SG.Genotype num2cell(Pval_per_SG.percentage_SG_Pval)...
        Pval_Latency_SD.Genotype num2cell(Pval_Latency_SD.Latency_after_SD_Pval)];
...
writematrix(Pval_data_all, save_location_Pval);
...
function ref_genotype_Callback(hObject, eventdata, handles)
% hObject    handle to ref_genotype (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ref_genotype as text
%        str2double(get(hObject,'String')) returns contents of ref_genotype as a double


% --- Executes during object creation, after setting all properties.
function ref_genotype_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ref_genotype (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
