...
setwd('X:/Sequencing_data/Mechanical_SD_Repeat_dataset_8-7-2024/Data_analysis_Shiju')

base_dir <- 'X:/Sequencing_data/Mechanical_SD_Repeat_dataset_8-7-2024/01.RawData/Kallisto_out'


# Sample name
sample_id <- dir(file.path(base_dir))
sample_id<-sample_id[c(1:3,10:12)] # only control samples and 6hr sd samples

# Location of alignment output for each sample
kal_dirs <- sapply(sample_id, function(id) file.path(base_dir, id))

# Create the sample-condtion information table (We have to made it based on experiment)
s2c<-data.frame(sample=(sample_id), condition=c(rep("_NonSD",3), rep("SD",3)))
...
so<- sleuth_prep(s2c, ~ condition, target_mapping = t2g, filter_fun=Quality_filter, aggregation_column = 'ext_gene', gene_mode = TRUE, extra_bootstrap_summary=T,
...
test_table_sig<-test_table[which(test_table$diffexpressed!="Not sig"),]

top_genes_1<- test_table_sig[1:3, "target_id"]
top_genes<-
...
write.csv(DGE_all, 'DEG_0hSDvs6hrSD.csv')
